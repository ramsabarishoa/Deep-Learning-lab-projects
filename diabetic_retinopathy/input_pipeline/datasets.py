...
class data_preparation:
    """Class containing functions for data loading """
    ...
    def balancing_dataset(self, DEBUG_INPUT_PIPELINE):
        """ Function to balance the dataset by over-sampling the minority class """

        N_Training = round(len(self.df_train) * 0.8)
        train = self.df_train[:N_Training]
        self.validation = self.df_train[N_Training:]
        print('---------------------------------------------------------')
...
dataset_path = input("Enter the path for the dataset and unzip the contents: ")
